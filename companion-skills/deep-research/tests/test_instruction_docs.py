...
class InstructionDocsTests(unittest.TestCase):
    ...
    def test_tool_routing_reference_exists_and_is_loaded_before_retrieval(self):
        ...
        self.assertIn('effort: "medium"', tool_routing)
        self.assertIn(
    ...
    def test_fresh_same_model_critique_is_documented(self):
        methodology = (ROOT / 'reference' / 'methodology.md').read_text()
        quality_gates = (ROOT / 'reference' / 'quality-gates.md').read_text()
        skill = (ROOT / 'SKILL.md').read_text()
        readme = (ROOT / 'README.md').read_text()

        self.assertIn('Phase 7.6: FRESH SAME-MODEL ADVERSARIAL REVIEW', methodology)
        self.assertIn('cross_model_critique.py run', methodology)
        self.assertIn('--reviewer claude --model [exact-live-selected-model]', methodology)
        self.assertIn('Claude Code WSL (`~/.claude`) | `claude`', methodology)
        self.assertIn('Full-mode Gauntlet is the sole explicit exception', methodology)
        self.assertIn('never rewrites locked primary artifacts', methodology)
        self.assertIn('Phase 8 is blocked until the review status is `ok`', methodology)
        self.assertIn('fresh top-level Claude Code instance', quality_gates)
        self.assertIn('FRESH SAME-MODEL ADVERSARIAL REVIEW', skill)
        self.assertIn('Fresh same-model adversarial review', readme)
    ...
    def test_role_effort_budgets_are_documented(self):
        ...
        self.assertIn('Use medium effort for CitationAuditor and GapAuditor', methodology)
        ...
        self.assertIn('REASONING_EFFORT: <medium by default', brief)
        self.assertIn('Role effort budgets', skill)
        self.assertIn('Gauntlet parent override', skill)
        self.assertIn('skip Phase 7.6', skill)
        ...
        self.assertIn('Full-mode Gauntlet runs skip Phase 7.6', readme)
    ...
    def test_consistency_sweep_contracts_are_documented(self):
        ...
        self.assertIn('Audit &rarr; Fresh Same-Model Review &rarr; Package', readme)
