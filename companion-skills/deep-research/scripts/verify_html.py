...
class HTMLVerifier:
    ...
    def _check_sections(self, html: str, md: str):
        ...
        md_sections = [
            section for section in re.findall(r'^## (.+)$', md, re.MULTILINE)
            if section.strip().lower() != 'bibliography'
        ]
        ...
        if len(md_sections) > len(html_sections):
            self.errors.append(
                f"Section count mismatch: MD has {len(md_sections)} sections, HTML has only {len(html_sections)}"
            )
    ...
    def _check_citations(self, html: str, md: str):
        ...
        md_citations = set(re.findall(r'\[(?:S|E)?\d+\]', md))
        ...
        html_citations = set(re.findall(r'\[(?:S|E)?\d+\]', html_content))
    ...
    def _check_bibliography(self, html: str, md: str):
        """Verify optional bibliography is formatted if explicitly present."""
