...
| 7.6 | FRESH SAME-MODEL ADVERSARIAL REVIEW | - | Optional* | Optional* | Optional* |
| 8 | PACKAGE | Y | Y | Y | Y |
...
\* Phase 7.6 is required rather than optional for high-stakes or investment-sensitive reports.

**Subagent fan-out per mode:** Quick 0 subagents (main thread inline); Standard 1 in a single wave; Deep 2 in a single wave; UltraDeep up to 4 research subagents spawned concurrently by default, using Sonnet 5 via `model: "claude-sonnet-5"` and `effort: "medium"` on each Agent call where supported. If runtime limits, quotas, or tooling prevent 4 concurrent workers, spawn the maximum available and continue in waves until 4 total research workers have run, then disclose the fallback. Source-volume targets: Quick 10+, Standard 25+, Deep 50+, UltraDeep 100-300+ unique sources.
...
**Role effort budgets:** Use `plan.json` lane `execution_budget` values when spawning workers. Every Claude research or audit worker defaults to Sonnet 5 (`claude-sonnet-5`) at `medium`; role-specific timeout and tool-call budgets still vary by lane.

**Gauntlet parent override:** When this skill runs inside full-mode Gauntlet Stage 1, skip Phase 7.6. Gauntlet Stage 2 retains its explicitly authorized different-model panel/judge.
...
2. **Phase 0.5-7.6:** Load [methodology.md](./reference/methodology.md) for detailed phase instructions, including clarify-or-brief, Phase 3 mode-scaled evidence-collection fan-out, Phase 7.5 CitationAuditor + GapAuditor, and Phase 7.6 fresh same-model adversarial review
...
- `python scripts/cross_model_critique.py run --dir [run_folder] --report [draft.md] --reviewer claude --model [exact-live-selected-model] --effort [same-or-higher-effort] --timeout 600` (legacy filename; launches a fresh top-level same-model Claude instance, never an Agent/subagent)
