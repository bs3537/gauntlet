...
9. Generate Methodology → Edit/append to file
10. Save source registry and evidence artifacts separately
...
- Persist full source metadata in sources.jsonl, not in the report body
- Use compact source labels in the report; avoid long bibliography/source sections unless explicitly requested by the user
- NO fabricated citations, NO unsupported factual claims
...
- Time period covered
- For biotech/pharma investment research: Native web search first, Search-as-Code second, Targeted direct Perplexity follow-ups third, Primary documents before conclusions; record provider coverage gaps and discrepancies, BioMCP/PubMed-PMC/Semantic Scholar/scite/FMP routing, pipeline-sweep status, and source freshness/retrieval dates
...
**Investment Evidence Separation (for biotech/pharma investment research):**
- Verified facts: [Facts directly supported by primary or high-quality sources]
- Source interpretation: [How cited sources frame the facts]
- Analyst inference: [Your inference from the evidence, labeled as inference]
- Market-implied expectations: [What valuation, price action, consensus, or financing terms appear to imply]
- Unresolved uncertainty: [What remains unknown or weakly evidenced]
...
**Evidence Artifacts:**
- Source registry: `sources.jsonl`
- Evidence ledger: `evidence.jsonl`
- Claim ledger: `claims.jsonl`

<!-- Do NOT append a full bibliography here unless the user explicitly asks for one. -->
...
**Source-Tier / Credibility Assessment:**
- How source quality was evaluated and assigned to `source_tier`
- Source-tier distribution: primary / high_quality_secondary / secondary / low_confidence / unknown
- `audit_manifest.json` warnings reviewed: `high_unknown_source_tier_ratio`, domain concentration, low-information evidence, or none
- Low-confidence load-bearing source count and remediation steps
...
| Claim ID | Major Claim | Claim Domain | Source Tier | Evidence Span | Status | Investment Relevance |
|----------|-------------|--------------|-------------|---------------|--------|----------------------|
| C001 | [First major claim from findings] | clinical / regulatory / financial / commercial / competitive / scientific / market | primary / high_quality_secondary / secondary / low_confidence | [Quote/span, source URL/name, document date, retrieved date] | verified / partially_verified / conflicting / not_found / stale | High / Medium / Low |
| C002 | [Second major claim] | [Domain] | [Tier] | [Evidence span] | [Status] | High / Medium / Low |
| C003 | [Third major claim] | [Domain] | [Tier] | [Evidence span] | [Status] | High / Medium / Low |
...
For bundled `claims.jsonl` support-verification scripts, preserve the script's legacy `claim_type` values (`factual`, `synthesis`, `recommendation`, `speculation`) and store the investment domain as `claim_domain`.
