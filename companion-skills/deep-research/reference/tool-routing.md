...
- Subagents: use the Agent tool when available, pin workers to Sonnet 5 at medium by default, pass `model: "claude-sonnet-5"` and `effort: "medium"` when overrides are supported, and embed or paraphrase `templates/subagent_brief_template.md` in every research-worker prompt.
