...
- If final sections: finalize source registry/evidence artifacts, verify report, cleanup state
"""
)
```
...
- Finalize `sources.jsonl`, `display_map.json`, `evidence.jsonl`, and `claims.jsonl`
...
- Finalize display labels: `python scripts/citation_manager.py assign-display-numbers --dir [run_folder] --write --order-from-report [path]`
- Before generating continuation prose, verify retrieval closure: read `coverage_map.json` and stop if any lane/query family is `planned`, `in_progress`, or `below_target` without `bounded`/`gap_disclosed` rationale. Read `outline_refinement.md` and continue only with sections listed there. After each appended section, write/update its per-section audit JSON under `audit/section_citation_issues/` before moving to the next section.
- Run strict semantic delivery gate: `python scripts/delivery_gate.py --dir [run_folder] --report [path] --strict --semantic --require-section-citation-audits`
- Stamp run completion: `python scripts/citation_manager.py finish-run --dir [run_folder] --report [path] --note "Delivery gate passed."`
