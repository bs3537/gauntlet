...
1. Merge each subagent's `[TOPIC_ID].evidence.jsonl` into the master ledgers with `python scripts/merge_subagent_evidence.py --dir [folder] --subagent-dir [folder]/subagent_outputs --strict`; this registers or reuses each source URL, maps it to `source_id`, then persists canonical evidence rows with `source_id`, `quote`, `locator`, `evidence_type`, and `retrieval_query`. `--strict` is mandatory: a lane whose handoff falls below `--min-usable-ratio` (default 0.8 accepted rows), or that yields zero valid rows, is a mechanical handoff failure, not a judgment call. On failure, record that lane `below_target` or `gap_disclosed` in `coverage_map.json` and never `covered` on the subagent's self-reported counts; the failure also flips the run status to `partial`. Inspect `per_file[].usable_ratio` in the merge summary to identify which lane failed before deciding whether to re-run it.
...
- **Independent verification of material claims (mandatory).** For every material claim, do not judge the researcher-stored quote: re-open the cited source yourself and record what you read there. A claim is material when `investment_relevance` is `high`, when the lead set `material: true`, or when its text pins a regulatory, trial, or filing state (approved, cleared, Phase 1/2/3, topline, primary endpoint, filed, accepted for review, CRL, PDUFA, recalled, withdrawn, discontinued). Write `verifier_quote` (the text you read on re-open), `verifier_locator` (page/section/table), `verifier_source_url` (the URL you actually opened, which may differ from the cited one when you cross-checked elsewhere), and `verified_independently_at` back to the claim row in `claims.jsonl`. Both `verifier_quote` and `verifier_locator` are required: a quote with no locator cannot be re-checked, and a locator with no quote records only that someone visited the page. `audit_manifest.py` counts material claims missing this pair as `material_claim_no_independent_evidence` -- a warning in Quick/Standard/Deep, and critical under UltraDeep strict delivery. Non-material claims keep the stored-quote entailment path and need no re-open.
